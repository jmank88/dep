...
type SourceManagerConfig struct {
	CacheAge       time.Duration // Maximum valid age of cached data. 0: always refresh. <0: no cache.
...
// is discarded. When cacheAge is 0, no persistent cache data will be valid, but
// new values will still be written. When cacheAge < 0, the persistent cache is
// not used.
//
...
// Release lets go of any resources held by the SourceManager. Once called, it is
// no longer safe to call methods against it; all method calls will immediately
// result in errors.
